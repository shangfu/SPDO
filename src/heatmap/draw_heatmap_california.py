...
def start(fname):
    ...
    for price, plat, plon in priced_points:
        ...
        if counts[x,y]==0:
            ...
            prices[x,y] += price
#            prices[x,y]=max(price, prices[x,y])
#        counts[x,y]=1
        counts[x,y] +=1
        #print x,y

    for x in prices:
        if prices[x] is not None:
            prices[x] = prices[x]*1.0/(counts[x]*1.0)

    radius = 2

    tmp = prices.copy()
    tmpcount = counts.copy()
    for x in range(MAX_X):
        for y in range(MAX_Y):
            if tmp[x,y] is not None:
                tot=0
                sums=0
                    ...
                    for j in range(int(radius*2+1)):
                        if abs(i-radius)+abs(j-radius)<=radius:
                            ...
                            if prices[xx,yy] is not None:
                                tot+=counts[xx,yy]
                                sums+=prices[xx,yy]*counts[xx,yy]
                if tot!=0:
                    tmp[x,y] = sums*1.0/(tot*1.0)
                    tmpcount[x,y]=tot
    prices=tmp.copy()
    counts=tmpcount.copy()
...
#    buckets = [40000
    ...
    for i in range(len(buckets)):
        buckets[i]=int(buckets[i]/1000)*1000
#    buckets = [400000, 250000, 200000, 170000, 150000, 140000, 130000, 115000, 105000, 95000, 85000, 75000, 65000, 55000, 45000, 25000, 10000, 2000]

#    buckets= [130000, 90000, 85000, 80000, 75000, 70000, 65000, 60000, 52000, 45000, 40000, 35000, 25000, 20000, 15000, 10000, 5000, 2000]

#    buckets = [50000, 35000, 30000, 25000, 23000, 22000, 20000, 19000, 18000, 16000, 15000, 14000, 12000, 11000, 9000, 7000, 5000, 2000]

    buckets = [150000, 120000, 100000, 90000, 80000, 70000, 65000, 60000, 56000, 52000, 48000, 44000, 40000, 36000, 32000, 28000, 24000, 20000]
